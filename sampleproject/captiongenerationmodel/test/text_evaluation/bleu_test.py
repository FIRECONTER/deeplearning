...
"""
Description:
    1) 模型使用了bleu 来进行评估 需要明白bleu这个指标表示什么然后如何计算
    2) 编码中使用nltk中的bleu 需要了解nltk 中的bleu 怎么使用
"""
import numpy as np
import re
...
if __name__ == '__main__':
    ...
    # 使用nltk 中的文本samples 做一个实验

# 小结
# 1.Bleu score 究竟是什么
# blue 是一个衡量两个句子相似度的指标,一般用于评估翻译质量。将当前的翻译结果与标准的翻译结果进行评分 然后对这些结果在整个语料库中进行平均计算 用于衡量
# 翻译的质量. bleu 适用于在整个语料库中用于衡量翻译的质量 取值范围在0-1 而不适用于在单个句子上进行打分
# 本质上讲是基于两个句子之间的共现词频率,适用一些算法技巧,最终用数值衡量两个句子之间的相似程度.
# 这个评估指标是简单并且快速的给出一个具有参考价值的指标 在快速选择模型的时候可以采用这种方法 但是它的准确度却比较差
# 评估方法具有两个缺陷 1) 常用词会使得翻译得分偏高 2) 短句也会使得翻译得分偏高 但是可以引入惩罚项适当的限制翻译短句得分.
# 所以当需要对翻译结果进行准确的评估的时候这个指标却不适用.